...
/** Draft stage: at or above this, a passage goes to the supervisor before submission. */
const ATTENTION_THRESHOLD = 10;

/**
 * The listing floor: matches at or above this are shown so a reader can judge a
 * weak overlap themselves. Mirrors MEANINGFUL_SIMILARITY in the scoring library.
 */
const MEANINGFUL_SIMILARITY = 8;
    ...
    <div className="overflow-hidden rounded-lg border border-border bg-surface shadow-sm">
      ...
      <div className="space-y-5 bg-background p-6">
        ...
        <p className="text-xs text-muted">
          Novelty is scored against the published literature — searched live across Semantic Scholar,
          OpenAlex, and Crossref — plus {archiveSize} archived{" "}
          {archiveSize === 1 ? "thesis" : "theses"} in the university repository. Draft similarity is
          scored against the archive only, since duplication of this university&apos;s own work is what
...
function NoveltyResult({ result }: { result: CheckResult }) {
  // Only matches that actually resemble the proposal count toward "similar
  // theses found" — a 6% top match is shared vocabulary, not covered ground.
  const similarCount = result.matches.filter((m) => m.score >= MEANINGFUL_SIMILARITY).length;
  ...
  return (
    <>
          ...
          <p className="mt-2 text-sm text-muted">
            ...
            {result.alreadyCovered.length > 0 ? (
              <>
                ...
                {similarCount > 0
                  ? ` (${similarCount} similar ${similarCount === 1 ? "thesis" : "theses"} found)`
                  : " — as vocabulary only; no archived thesis closely resembles this."}
              </>
            ) : (
      ...
      {result.matches.length > 0 ? (
        <MatchTable result={result} title="Closest archived thesis" />
      ) : (
        <NoMatches
          heading="Closest archived thesis"
          message={
            result.archiveSize === 0
              ? "No theses are archived yet, so there is nothing local to compare against."
              : `No archived thesis matches this proposal. All ${result.archiveSize} were checked; none scored above ${MEANINGFUL_SIMILARITY}% similarity, which is the point below which an overlap is shared vocabulary rather than related work.`
          }
        />
      )}
      {result.externalMatches?.length > 0 ? (
        <ExternalTable matches={result.externalMatches} />
      ) : (
        <NoMatches
          heading="Closest published papers"
          message="No published paper matched this closely enough to be worth listing."
        />
      )}
    </>
  );
...
/**
 * Said plainly rather than padded with weak rows. Listing a 3% match under
 * "closest" invents a relationship the reader has no way to check, and erodes
 * trust in the matches that are real.
 */
function NoMatches({ heading, message }: { heading: string; message: string }) {
  return (
    <div className="rounded-lg border border-border bg-surface p-5">
...
/**
 * The published papers this proposal actually overlaps with. Every row links
 * out, because a similarity figure the student cannot go and read gives them no
 * way to judge whether it truly covers their idea.
 */
function ExternalTable({ matches }: { matches: ExternalMatch[] }) {
  return (
    <div className="rounded-lg border border-border bg-surface p-5">
      <h2 className="font-serif text-lg font-semibold text-foreground">Closest published papers</h2>
      ...
      <div className="mt-4 overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {matches.map((m, i) => (
              <tr key={`${m.title}-${i}`} className="border-b border-border last:border-0">
                <td className="py-3 pr-4">
                  ...
                  <p className="mt-0.5 text-xs text-muted">
                    {[m.venue, m.year].filter(Boolean).join(" · ") || "Venue not recorded"}
                  </p>
  ...
  return (
    <>
      {result.matches.length > 0 ? (
        ...
        <NoMatches
          heading="TF-IDF cosine similarity"
          message={
            result.archiveSize === 0
              ? "No theses are archived yet, so there is nothing local to compare this draft against."
              : `No archived thesis matches this draft. All ${result.archiveSize} were checked; none scored above ${MEANINGFUL_SIMILARITY}% similarity.`
          }
        />
      )}
      ...
      {result.externalMatches?.length > 0 ? (
        ...
        <NoMatches
          heading="Closest published papers"
          message="No published paper overlaps this draft closely enough to be worth listing."
        />
      )}
      ...
      <p className="text-xs text-muted">
        ...
        {result.externalSize > 0
          ? `, against ${result.archiveSize} archived theses and ${result.externalSize} published papers.`
          : "."}
      </p>
    </>
  );
